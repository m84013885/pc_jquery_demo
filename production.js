...
const extractCss = new ExtractTextWebpackPlugin('css/[name].[hash:5].css')
const {
  routers,
  publicPath
} = require('./config.json')
const webpackConfig = {
  ...
  module: {
        ...
        use: [
          'css-loader?modules&localIdentName=[name]_[local]',
          {
            ...
            options: {
              ident: 'postcss',
              config: {
                path: path.resolve(__dirname, 'dev/postcss.config.js')
              }
            }
          }
        ],
    ...
    },
    {
      ...
      use: [{
        loader: `url-loader?limit=25000&name=[name].[ext]&outputPath=./image/&publicPath=../image`
      }],
      include: [path.resolve(__dirname, 'app')],
      exclude: [nodeModuleDir]
    }
